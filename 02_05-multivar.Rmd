...
We will use two different `r emo::ji("package")` to fit more complex models that are not possible with  `lmer()` from `lme4` 
`r emo::ji("package")` [@R-lme4]. We will use:

- `asreml-R` which is a commercial software developped by VSNi [@R-asreml]. `ASReml` fit models using a maxiumum likelihood approach, is quite flexible and fast.
- `MCMCglmm` which is free and open-source and fit model using a Bayesian approach [@MCMCglmm2010]. It is super flexible and allow to fit a wide diversity of distribution.
...
```{r loadlibs_mm, message=FALSE, results='hide', warning=FALSE}
...
```{r, out.width = "50%", echo = FALSE, fig.align = "center", fig.cap = "Blue dragon male"}
...
We first use the `lme4` `r emo::ji("package")` to determine the proportion of phenotypic variation (adjusted for fixed effects) that is due to differences among individuals, separately for each trait with repeated measures. 
...
```{r, fig.cap = "Checking assumptions of model lmer_f"}
...
```{r, fig.cap = "Checking assumptions of model lmer_e"}
...
```{r, fig.cap = "Relation between exploration and flying speed using BLUPs from univariate models"}
...
As you can see, we get a positive correlation with a very small p-value (P = `r print(cor_blups$p.value, digits = 3)`), indicating that these traits are involved in a behavioural syndrome. While the correlation itself is fairly weak ($r = `r print(cor_blups$estimate, digits = 3)`), it appears to be highly significant, and suggests that individuals that are faster than average also tend to be more exploratory than average.
...
```{r, fig.cap = "Relation between exploration and flying speed using BLUPs from univariate models including +/- SE as error bars"}
ggplot(df_blups_fe, aes(x = exploration, y = speed)) +
  geom_point() +
  geom_linerange(aes(
...
```{r, fig.cap = "Checking assumptions of model asr_us"}
par(mfrow = c(1, 3))
...
```{r, echo = FALSE, eval = htmlF_pdfT, purl = FALSE}
summary(asr_us, coef = TRUE)$coef.fixed
wa <- wald(asr_us, ssType = "conditional", denDF = "numeric")$Wald
...
However, if we consider confidence intervals as $+/- 1.96SE$ around the estimate, the lower bound of the confidence interval would actually be `r print(cor_fe[1] - cor_fe[2]*1.96, digits = 3)`.
...
```{r, fig.cap = "Correlation estimates (with CI) using 2 different methods"}
df_cor <- data.frame(
...
Here we can clearly see that the BLUPs method - having failed to carry through the error around the predictions of individual-level estimates - is anticonservative, with small confidence intervals and a correspondingly small P-value ($P = `r print(cor_blups$p.value, digits = 3)`$). Testing the syndrome directly in a bivariate model that retains all the data, by
comparison, enables us to capture the true uncertainty about the estimate of the correlation. This is reflected
in the larger confidence intervals and, in this case, the non-significant P-value ($P = `r print(p_biv, digits = 3)`$).
...
```{r, fig.cap = "MCMC trace and Posterior distribution of the (co)variance estimates of model mcmc_us"}
...
```{r, fig.cap = "Posterior trace and distribution of the repeatability in flying speed"}
...
```{r, fig.cap = "Posterior trace and distribution of the repeatbility of exploration"}
...
```{r, fig.cap = "Posterior trace and distribution of the correlation between flying speed and exploration"}
...
```{r, fig.cap = "Correlation estimates (with CI) using 3 different methods"}
df_cor[3, 1] <- "MCMCglmm"
...
knitr::kable(df_cor,
  digits = 3,
  caption = "Correlation (with 95% intervals) between flying speed and exploration estimated with 3 different methods"
)
```
