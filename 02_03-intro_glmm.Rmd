...
```{r, echo = FALSE, fig.cap = "Number of fruits (log + 1) as a function of treatments"}
p1 <- qplot(
...
dat_tf <- dat_tf %>%
  mutate(
    ...
    gna = fct_reorder(gna, total.fruits, mean)
  )
```
...
ggplot(dat_tf, aes(x = gna, y = log(total.fruits + 1))) +
  geom_boxplot() +
  theme_bw() +
