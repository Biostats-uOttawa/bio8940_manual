...
 We use `mutate()` `dplyr` `r emo::ji("package")`, which operates within the data set, to avoid typing lots of commands like `dat_tf$rack <- factor(dat_tf$rack)`
...
1. Do you find the mosaic plot you obtained ugly and super hard to read? Me too `r emo::ji("laugh")`
```{r, fig.cap ="A truly useless plot no one can understand"}
plot(reptab)
```
...
```{r, echo = FALSE, fig.cap = "Number of fruits (log + 1) as a function of treatments", purl = FALSE}
p1 <- qplot(
...
```{r, eval = FALSE, fig.cap = "Number of fruits (log + 1) as a function of treatments"}
p1 <- qplot(
    interaction(nutrient, amd),
    log(1 + total.fruits),
    data = dat_tf, geom = "boxplot") +
...
```{r, fig.cap = "Boxplot of total fruits (log + 1) per genotypes and treatments"}
...
```{r, fig.cap = "Graphical evaluation of distribution to use"}
...
Same with ggplot
```{r, fig.cap = "Graphical evaluation of distribution to use with ggplot"}
ggplot(
...
```{r, fig.cap = "Graphical evaluation of distribution to use including quadratic effect"}
...
```{r, fig.cap = "Fruit production by treatments by population"}
...
```{r, fig.cap = "Fruit production by genotype by treatments"}
...
```{r, fig.cap = "Model coefficients for GLM fits on each genotype"}
glm_lis <- lmList(
...
```{r, fig.cap = "Q-Q plots of model coefficients for GLM fits on each genotype"}
...
We can also use the excellent `DHARMa` `r emo::ji("package")` [@R-DHARMa] to evaluate fit of *glm* and *glmm*. So instead of using the function `overdisp_fun()`, we can simply use the function `testDispersion()`.
...
Using the `DHARMa` `r emo::ji("package")`, we will also check the model. To do so we first need to simulate some data and get the *scaled residuals* following the DHARMa notation.
...
```{r simdev_glmm, eval = FALSE, echo = TRUE}
simdev <- function() {
...
```{r simdev_glmm_saved, eval = params$longrun, echo = FALSE, include = FALSE, purl = FALSE}
simdev <- function() {
  newdat <- simulate(mp4v1)
  reduced <- lme4::refit(mp4v1, newdat)
  full <- lme4::refit(mp4, newdat)
...
obsdev <- 2 * c(logLik(mp4) - logLik(mp4v1))
```
```{r, echo = FALSE, include = FALSE, purl = FALSE, eval = !params$longrun}
...
```{r, fig.cap = "Distribution of BLUPs for genotypes and populations"}
...
```{r, fig.cap = "Q-Q plot of BLUPs from model mp5"}
