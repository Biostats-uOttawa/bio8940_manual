...
```{r create_data, echo = FALSE, eval=TRUE}
create_data <- function(
  ...
  y <- (intercept + u[cluster, 'Intercept']) + (beta + u[cluster, 'x'])*x + rnorm(N, sd = sigma)
...
```{r create_plot_data, echo=FALSE, eval=FALSE}
...
ggsave('images/shrinkage/baseline_static.svg')
```

```{r baseline_gif, echo=F, eval=T, dev.args=list(bg = 'transparent'), cache=FALSE}
knitr::include_graphics('images/shrinkage/baseline.gif')
```
...
What happens when we add more subject level variance relative to the residual variance?  The mixed model will show relatively less shrinkage, and what were previously less probable outcomes are now more probable, and thus opting for the clusters to speak for themselves.

```{r add_subject_var, echo = F, eval = F}
...
plot_fun(plot_data = plot_data,
         ...
         duration = 12,
         nframes = 120)
anim_save('shrinkage/add_subject_level_variance.gif')

plot_fun(plot_data = plot_data, animate = F)
ggsave('shrinkage/add_subject_level_variance_static.svg')
```

```{r subject_level_var_gif, echo=F, eval=F, dev.args=list(bg = 'transparent'), cache=FALSE}
knitr::include_graphics('shrinkage/add_subject_level_variance.gif')
```

<aside><img src="shrinkage/add_subject_level_variance_static.svg" style="display:block; margin: 0 auto; width: 250%"></aside>
...
plot_fun(plot_data = plot_data,
         ...
         duration = 12,
         nframes = 120)
anim_save('shrinkage/more_slope_var.gif')

plot_fun(plot_data = plot_data, animate = F)
ggsave('shrinkage/more_slope_var_static.svg')
```


```{r slope_level_var_gif, echo=F, eval=F, dev.args=list(bg = 'transparent'), cache=FALSE}
knitr::include_graphics('shrinkage/more_slope_var.gif')
```
...
```{r fewer_observations, echo=F, eval=F}
...
plot_fun(plot_data = plot_data,
         ...
         duration = 12,
         nframes = 120)
anim_save('shrinkage/fewer_obs.gif')

plot_fun(plot_data = plot_data, animate = F)
ggsave('shrinkage/fewer_obs_static.svg')
```

```{r fewer_observations_gif, echo=F, eval=F, dev.args=list(bg = 'transparent'), cache=FALSE}
knitr::include_graphics('shrinkage/fewer_obs.gif')
```

<aside><img src="shrinkage/fewer_obs_static.svg" style="display:block; margin: 0 auto; width: 250%"></aside>
...
```{r more_observations, echo=F, eval=F}
...
plot_fun(plot_data = plot_data,
         ...
         duration = 12,
         nframes = 120)
anim_save('shrinkage/more_obs.gif')

plot_fun(plot_data = plot_data, animate = F)
ggsave('shrinkage/more_obs_static.svg')
```

```{r more_observations_gif, echo=F, eval=F, dev.args=list(bg = 'transparent'), cache=FALSE}
knitr::include_graphics('shrinkage/more_obs.gif')
```

<aside><img src="shrinkage/more_obs_static.svg" style="display:block; margin: 0 auto; width: 250%"></aside>
...
anim_save('shrinkage/unbalanced.gif')

plot_fun(plot_data = plot_data, animate = F)
ggsave('shrinkage/unbalanced_static.svg')
```

```{r unbalanced_gif, echo=F, eval=F, dev.args=list(bg = 'transparent'), cache=FALSE}
knitr::include_graphics('shrinkage/unbalanced.gif')
```

<aside><img src="shrinkage/unbalanced_static.svg" style="display:block; margin: 0 auto; width: 250%"></aside> -->
