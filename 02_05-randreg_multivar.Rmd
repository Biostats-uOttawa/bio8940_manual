# Random regressions and multivariate mixed models
...
As you can see, we get a positive correlation with a very small p-value ($P = `r cor_blups$p.value`$), indicating that these traits are involved in a behavioural syndrome. While the correlation itself is fairly weak ($r = `r cor_blups$estimate`), it appears to be highly significant, and suggests that individuals that are faster than average also tend to be more exploratory than average.
...
```{r}
summary(asr_us, all = TRUE)$coef.fixed
```
...
However, if we consider confidence intervals as $+/- 1.96SE$ around the estimate, the lower bound of the confidence interval would actually be `r cor_fe[1] - cor_fe[2]*1.96`.
...
Here we can clearly see that the BLUPs method - having failed to carry through the error around the predictions of individual-level estimates - is anticonservative, with small confidence intervals and a correspondingly small P-value ($P = `r cor_blups$p.value`$. Testing the syndrome directly in a bivariate model that retains all the data, by
comparison, enables us to capture the true uncertainty about the estimate of the correlation. This is reflected
in the larger confidence intervals and, in this case, the non-significant P-value ($P = `r p_biv`).
