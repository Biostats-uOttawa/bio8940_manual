...
1. it is blazing fast `r emo::ji("fast")` (for Bayesian analysis) for some models particularly models with structured covariances
2. it is fairly intuitive to code

**but** it also has some inconvenients:

1. it is blazing fast for **Bayesian analysis** meaning it is `r emo::ji("snail")` compared to *maximum likelihood* approaches
...
3. the priors, *oh, the priors* `r emo::ji("loudly_crying_face")`, are a bit tricky to code and understand `r emo::ji("exploding_head")`.
...
```{r, fig.cap = "Posterior trace and distribution of the paremeters in m_mcmcglmm using default settings"}
plot(m_mcmcglmm$Sol)
plot(m_mcmcglmm$VCV)
...
```
 evaluate model here
```{r, fig.cap = "Posterior trace and distribution of the paremeters in m_mcmcglmm with better settings"}
plot(m_mcmcglmm$Sol)
plot(m_mcmcglmm$VCV)
autocorr.diag(m_mcmcglmm$VCV)
```
...
```{r, cache = TRUE, fig.cap = "Autocorrelation in the chain for variance parameters in model m_brm"}
...
```{r, fig.cap = "Fixed effect estimates (with 95% credible intervals) from model m_brm"}
summary(m_brm)
...
```{r, fig.cap = "Among-individual and residual standard deviance ( with 95% credible intervals) estimated from model m_brm"}
summary(m_brm)
...
```{r, out.width = "50%", echo = FALSE, fig.align = "center", fig.cap = "Sherlock Holmes, a truly bayesian detective"}
