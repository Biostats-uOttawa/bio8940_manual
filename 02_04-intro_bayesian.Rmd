...
for (i in 2:n_iter) {
  proposal <- pd_mean[i - 1] + mvrnorm(1, 0, 5) # proposal
...
for (j in seq_len(n_chain)) {
  ...
  for (i in 2:n_iter) {
    proposal <- pd_mean[i - 1, j, 1] + mvrnorm(1, 0, 5) # proposal
...
1. it is blazing fast `r emo::ji("fast")` (for Bayesian analysis) for some models
